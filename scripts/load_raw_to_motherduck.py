"""Load Open-Meteo raw CSV extracts into MotherDuck raw tables."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-dir",
        default="data/raw",
        help="Directory containing raw_locations.csv and related raw CSV files.",
    )
    parser.add_argument(
        "--database",
        default=None,
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Truncate each raw table before loading instead of upserting by natural key.",
    )
...
def motherduck_path() -> str:
    return f"md:?motherduck_token={quote(get_token(), safe='')}"
...
def run_bootstrap_sql(connection: duckdb.DuckDBPyConnection, database: str) -> None:
...
def create_temp_view(
    ...
    csv_path_literal = sql_literal(csv_path.resolve().as_posix())
    connection.execute(
        f"""
...
def load_table(
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"Missing required raw extract file: {csv_path}")
...
def main() -> int:
    ...
    try:
        run_bootstrap_sql(connection, database)
